export const TrackBodyCheck = (req, res, next) => {
    ...
    if(!title || title.length == 0) return res.status(400).json({ error: "Title should not be empty."});
    if(!description || description.length == 0) description == " "; 
    if(!music || music.length == 0) return res.status(400).json({ error: "Music should not be empty."});
