...
export const RemoveLike = async (req, res) => {
  try {
    ...
    const thelike = await prisma.like.findUnique({
      where: {
        trackid_authorid: { trackid: thetrack.id, authorid: currentuser.id },
      },
    });
    ...
    await prisma.like.delete({
      where: {
        trackid_authorid: { authorid: currentuser.id, trackid: thetrack.id },
      },
    });
