...
export const AddToPlaylist = async (req, res) => {
  try {
    ...
    const TheTrack = await prisma.track.findUnique({
      where: { slug: trackid },
    });
    ...
    const is_playlist_there = await prisma.playlist.findUnique({
    ...
    const is_songthere = is_playlist_there.tracks.some(
      (track) => track.id === TheTrack.id
    );
    if (is_songthere) throw new Error("Song already in playlist.");

    const user_playlist = await prisma.playlist.update({
      where: { id: is_playlist_there.id },
      include: { tracks: true },
      data: { tracks: { connect: { id: TheTrack.id } } },
    });

    return res
      .status(201)
      .json({ message: "Song added to playlist.", success: true });
  } catch (err) {
